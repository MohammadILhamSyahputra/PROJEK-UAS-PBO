...
# Form implementation generated from reading ui file 'form_riwayat_admin.ui'
#
...
"")
        ...
        self.btn_user_expanded.setChecked(False)
...
"}")
        ...
        # Inisialisasi format dan header
        self.tableWidget.setColumnCount(4)
        self.tableWidget.setHorizontalHeaderLabels(["Kode Transaksi", "Tanggal", "Kasir", "Total Nota (Rp)"])
    ...
    def set_kasir_dropdown(self):
        """Mengambil daftar kasir dan mengisi comboBox_2."""
        tr_instance = Transaksi()
        kasir_list = tr_instance.get_all_kasir_usernames()
        
        self.comboBox_2.clear()
        self.comboBox_2.addItem("Semua Kasir") # Opsi untuk melihat semua
        
        for username in kasir_list:
            self.comboBox_2.addItem(username)

    def set_default_date_range(self):
        """Mengatur rentang tanggal default (1 Desember 2025 s.d. hari ini, berdasarkan data Anda)."""
        today = QtCore.QDate.currentDate()
        # Mengatur tanggal awal menjadi 2025-12-01 (atau tanggal lain yang valid)
        default_start = QtCore.QDate(2025, 12, 1) 
        self.dateEdit.setDate(default_start) 
        self.dateEdit_3.setDate(today) 

    # --- C. Method Pemuat Data Utama ---

    def display_riwayat(self):
        kasir_pilihan = self.comboBox_2.currentText()
    
        # Tentukan filter: Jika "Semua Kasir" dipilih, gunakan "ALL"
        username_filter = kasir_pilihan if kasir_pilihan != "Semua Kasir" else "ALL"
        
        tgl_awal = self.dateEdit.date().toString("yyyy-MM-dd")
        tgl_akhir_obj = self.dateEdit_3.date().addDays(1)
        tgl_akhir = tgl_akhir_obj.toString("yyyy-MM-dd")
        
        tr_instance = Transaksi()
        
        try:
                # 1. Ambil Data RIWAYAT DETAIL PER ITEM
                riwayat_data = tr_instance.get_riwayat_admin(username_filter, tgl_awal, tgl_akhir)
                
                # 2. Hitung Total Keseluruhan (Tetap gunakan calculate_total_admin)
                grand_total = tr_instance.calculate_total_admin(username_filter, tgl_awal, tgl_akhir)
                
                # 3. Isi Tabel
                self.fill_riwayat_table(riwayat_data)
                
                # 4. Tampilkan Total
                self.lineEdit.setText(f"{grand_total:,.0f}")
                
        except Exception as e:
                QtWidgets.QMessageBox.critical(self.centralwidget, "DB Error", f"Gagal memuat riwayat: {e}")

    # --- D. Method Pengisi Tabel ---

    def fill_riwayat_table(self, data):
        table = self.tableWidget
        table.setRowCount(0)
        
        # Header untuk tampilan detail item (7 Kolom)
        headers = [
